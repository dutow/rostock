...
module screws() {
  ...
  for (x = [-1, 1]) {
	...
	poly_cylinder(r=screw_rad, h=side_thickness+2, center=true);
      // M3 nut holder.
      translate([-x*(side_thickness/2-nut_depth/2+0.1), -smooth_rod_radius-2*screw_rad, 0])
...
module bracket(h,fins=false) {
  translate([0, 0, -h/2]) linear_extrude(height=h, convexity=3) difference() {
    union() {
      translate([0, 3-smooth_rod_radius]) square([smooth_rod_separation+side_thickness, smooth_rod_radius + 18], center=true);
      for (x = [-smooth_rod_separation/2, smooth_rod_separation/2]) {
        // Sandwich mount.
        translate([x, 10]) circle(r=side_thickness/2, center=true);
    ...
    difference() {
      union() {
	translate([0, -5]) square([smooth_rod_separation-side_thickness-2*round_rad, 20], center=true);
	translate([0, -9]) square([smooth_rod_separation-side_thickness, 20], center=true);
        for (x = [-1, 1]) {
	  translate([x*(smooth_rod_separation/2-side_thickness/2-round_rad), 5-round_rad]) circle(r=round_rad, center=true);
        }
      }
      // Smooth rod mounting slot extra.
      for (x = [-smooth_rod_separation/2, smooth_rod_separation/2]) {
